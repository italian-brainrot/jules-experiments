...
if __name__ == "__main__":
    ...
    # --- PPE(Adam) Optimizer ---
    print("\n--- Training with PPE(Adam) ---")
    model_ppe = copy.deepcopy(initial_model)
    base_optimizer_ppe = torch.optim.Adam(model_ppe.parameters(), lr=1e-3)
    ...
    plt.plot(losses_ppe, label='PPE(Adam)')
    plt.xlabel('Epochs')
    plt.ylabel('Validation Loss')
    plt.title('Adam vs. PPE(Adam) Optimizer Performance')
    plt.legend()
    plt.grid(True)
